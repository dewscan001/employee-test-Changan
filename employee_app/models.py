...
class EmployeeModel(models.Model):
    ...
    position = models.ForeignKey(verbose_name="position", to='PositionModel', on_delete=models.SET_NULL,  blank=True, null=True)
    status = models.ForeignKey(verbose_name="status", to='StatusModel', on_delete=models.SET_NULL,  blank=True, null=True)
    image = models.ImageField(verbose_name="image", upload_to="image", blank=True, null=True)
    department = models.ForeignKey(verbose_name="department", to='DepartmentModel', on_delete=models.SET_NULL,  blank=True, null=True)
    def __str__(self):
        return self.name

class DepartmentModel(models.Model):
    department_name = models.CharField(verbose_name="Department", max_length=255, blank=False, null=False, unique=True)
    manager = models.OneToOneField(verbose_name="manager", to='EmployeeModel', on_delete=models.SET_NULL,  blank=True, null=True, limit_choices_to={'manager': True})
    def __str__(self):
